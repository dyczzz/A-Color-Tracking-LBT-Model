...
int main(int argc, char* argv[]){	  				
    ...
    const double jetetacut = 0.7, jetptmin = 60.0, jetptmax = 80.0, leadingptcut = 0.0;
...
//        cout << dummyInt << "  " << numParton << endl;
        ...
        for(int i=1; i<=numParton; i++) //negative
        {
            ...
            if (pythia.particleData.chargeType(pid) != 0) {
...
//cc	cout << "the constructed jet size is: " << "  " << inclusive_jets0001.size() << endl;
        ...
        for (int ij = 0; ij < inclusive_jets0001.size(); ij++) {
            ...
            if (inclusive_jets0001[ij].user_index()>=0)
            {
                ...
                {   
                    id_i=constituents[i].user_index();
                    if (id_i < 0) continue;
                    px_i=constituents[i].px();
                    py_i=constituents[i].py();
                    pz_i=constituents[i].pz();
                    ...
                    {
                        if (i == j) continue;
                        id_j=constituents[j].user_index();
                        if (id_j < 0) continue;
                        px_j=constituents[j].px();
                        py_j=constituents[j].py();
                        pz_j=constituents[j].pz();
                        pt_j=sqrt(px_j*px_j+py_j*py_j);
                        if (pt_j <= 1) continue;
                        p_j=sqrt(px_j*px_j+py_j*py_j+pz_j*pz_j);

                        theta_ij = acos((px_i*px_j + py_i*py_j + pz_i*pz_j)/p_i/p_j);
                        p_ij= constituents[i].pt()*constituents[j].pt();
                        for (int k=0; k<23; k++)
                        {
                            if ((theta_ij>=atlasbin[k])&&(theta_ij<atlasbin[k+1]))
                            {
                                numev[k] = numev[k] + p_ij/(ldpt*ldpt);
                            }
                        }
                    }
