...
int main(int argv, char* argc[])
{
    ...
    output_filename2 = path+"hadrons-nega.dat";// output files of final hadrons
...
// ****************** append the partons into pythia event *********************//
        ...
        for (int tt=0;tt<Npart;tt++){
            if(idpo[tt]==21){mass[tt]=0.0;}
                ...
                if(abs(idp[tt])<=2)mass[tt]=0.330;
                else if(abs(idp[tt])==3)mass[tt]=0.50;
                else if(abs(idp[tt])==4)mass[tt]=1.50;
                else if(abs(idp[tt])==5)mass[tt]=4.80;
                epo[tt]=sqrt(abs(mass[tt]*mass[tt]+pzpo[tt]*pzpo[tt]+pypo[tt]*pypo[tt]+pxpo[tt]*pxpo[tt]));
            }
            pythia.event.append(idpo[tt],62,nncol[tt],aacol[tt],pxpo[tt],pypo[tt],pzpo[tt],epo[tt],mass[tt]);
