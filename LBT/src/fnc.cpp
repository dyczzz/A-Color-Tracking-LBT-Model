...
// duplicate for heavy quark
     ifstream f11("tables/ratedata-HQ"); 		//for c quark
     ifstream f11b("b-tables/ratedata-HQ"); 		//for b quark
     if(!f11.is_open()||!f11b.is_open())
     {
     ...
     else
     {
	  ...
	  f11>>RHQ[it][ie]>>RHQ11[it][ie]>>RHQ12[it][ie]>>qhatHQ[it][ie];	      
          f11b>>itb>>ieb;
          f11b>>RHQb[itb][ieb]>>RHQ11b[itb][ieb]>>RHQ12b[itb][ieb]>>qhatHQb[itb][ieb];
	  }
     }
     f11.close();
     f11b.close();
// read radiation table for heavy quark
     ifstream f12("tables/dNg_over_dt_cD6.dat");
     ifstream f12b("b-tables/dNg_over_dt_bD6.dat");
     ...
     for(int k=1; k<=t_gn; k++) {
         ...
         f12b >> dummyChar >> dummyChar >> dummyChar >> dummyChar;
         ...
         for(int i=1; i<=temp_gn; i++) {
            ...
            for(int j=1; j<=HQener_gn; j++) {
                ...
                f12b >> dNg_over_dt_b[k+1][i][j] >> max_dNgfnc_b[k+1][i][j];
...
// preparation for HQ 2->2
     ifstream fileB("tables/distB.dat");
     ifstream fileBb("b-tables/distB.dat");
     if(!fileB.is_open()||!fileBb.is_open()) {
        cout << "Erro openning data file distB.dat!" << endl;
     } else {
        for(int i=0;i<N_T;i++) {
           for(int j=0;j<N_p1;j++) {
              double dummy_T,dummy_p1,dummy_Tb,dummy_p1b;
              fileB>>dummy_T>>dummy_p1;
              fileBb>>dummy_Tb>>dummy_p1b;
              ...
              fileB>>distFncBM[i][j];
              fileBb>>distFncBMb[i][j];
              for(int k=0;k<N_e2;k++) fileB>>distFncB[i][j][k];
              for(int k=0;k<N_e2;k++) fileB>>distMaxB[i][j][k];
              for(int k=0;k<N_e2;k++) fileBb>>distFncBb[i][j][k];
              for(int k=0;k<N_e2;k++) fileBb>>distMaxBb[i][j][k];
           }
        }
     }
     fileB.close();
     ...
     ifstream fileF("tables/distF.dat");
     ifstream fileFb("b-tables/distF.dat");
     if(!fileF.is_open()||!fileFb.is_open()) {
        ...
        for(int i=0;i<N_T;i++) {
           for(int j=0;j<N_p1;j++) {
              ...
              fileFb>>distFncFMb[i][j];
              for(int k=0;k<N_e2;k++) fileFb>>distFncFb[i][j][k];
              for(int k=0;k<N_e2;k++) fileFb>>distMaxFb[i][j][k];
           }
        }
     }
     fileF.close();
...
void lam(int KATT0,double &RTE,double E,double T,double &T1,double &T2,double &E1,double &E2,int &iT1,int &iT2,int &iE1,int &iE2){   
  ...
  if(KATT0==21) {	
	  ...
	  RTE=(RTE2-RTE1)*(E-E1)/(E2-E1)+RTE1;	  
  } else if (KATT0==5||KATT0==-5) { // add heavy quark channel
...
void flavor(int &CT,int &KATT0,int &KATT2,int &KATT3,double RTE,double E,double T,double &T1,double &T2,double &E1,double &E2,int &iT1,int &iT2,int &iE1,int &iE2){  
  ...
  // output: KATT0 for jet parton, KATT2 for recoil, KATT3 for negative
...
//	  qhatTP=(RTE2-RTE1)*(E-E1)/(E2-E1)+RTE1;	
  ...
  } else if(KATT==5||KATT==-5){  // add heavy quark channel
...
/***************************************************************************/
		...
		vvc[1]=(pp0[1]+pp2[1])/(pp0[0]+pp2[0]);
		vvc[2]=(pp0[2]+pp2[2])/(pp0[0]+pp2[0]);
		vvc[3]=(pp0[3]+pp2[3])/(pp0[0]+pp2[0]);
		...
		ranphi=2.0*pi*ran0(&NUM1);
		qqt=sqrt(pow(ecm,2)-pow((tt/2.0/ecm-ecm),2));
		qqx=qqt*cos(ranphi);
		qqy=qqt*sin(ranphi);

		qqpar=tt/2.0/ecm;

		uupt=sqrt(pp2[1]*pp2[1]+pp2[2]*pp2[2])/pp2[0];
		uupx=pp2[1]/pp2[0];
		uupy=pp2[2]/pp2[0];
		uupz=pp2[3]/pp2[0];

		pp2[1]=pp2[1]-qqpar*uupx;
		pp2[2]=pp2[2]-qqpar*uupy;
		if(uupt!=0.0)
		{
			pp2[1]=pp2[1]+(uupz*uupx*qqy+uupy*qqx)/uupt;
			pp2[2]=pp2[2]+(uupz*uupy*qqy-uupx*qqx)/uupt;
		}
		pp2[3]=pp2[3]-qqpar*uupz-uupt*qqy;
		...
		rotate(p4[1],p4[2],p4[3],pp0,1);
		qperp2=pow(pp0[1],2)+pow(pp0[2],2);
///ccc		qperp=sqrt(pow(pp0[1],2)+pow(pp0[2],2));

///ccc		cout << "Now in colljet22, Nqperp is: " << "    " << Nqperp2 << endl;
///ccc		cout << "T condition: " << "    " << (10.0*temp)*(10.0*temp) << "    " << "qperp2 is: " << qperp2 << endl;
/////cc		cout << "qperp2 is: " << qperp2 << endl;

/***************************************************************************/

////	  }while((tt<qhat0ud) || (tt>(ss-qhat0ud))); 
	  }while((tt<qhat0ud) || (tt>(ss-qhat0ud)) || (qperp2>(10.0*temp)*(10.0*temp))); 
  ...
  //    in cm frame
  //
////cccc  double ranp=2.0*pi*ran0(&NUM1);
  double ranp=ranphi;
  //
...
//.........................................................................
void collHQ22(int CT,double temp,double qhat0ud,double v0[4],double p0[4],double p2[4],double p3[4],double p4[4],double &qt,int parID){
  //
  //    HQ 2->2 scatterings
...
//           maxValue=distMaxF[index_T][index_p1][index_e2];
           if (fabs(parID) == 4) ff=distFncF[index_T][index_p1][index_e2]/fFmax;
           if (fabs(parID) == 5) ff=distFncFb[index_T][index_p1][index_e2]/fFmaxb;
           if (fabs(parID) == 4)  maxValue=distMaxF[index_T][index_p1][index_e2];
...
//           maxValue=distMaxB[index_T][index_p1][index_e2];
           if (fabs(parID) == 4)  ff=distFncB[index_T][index_p1][index_e2]/fBmax;
           if (fabs(parID) == 5)  ff=distFncBb[index_T][index_p1][index_e2]/fBmaxb;
           if (fabs(parID) == 4)   maxValue=distMaxB[index_T][index_p1][index_e2];
...
//              cout << sE1-sq0-sk0 << "  " << sp1x-sqx-skx << "  " << sp1y-sqy-sky << "  " << sp1z-sqz-skz << "  " << sE2+sq0 << "  " << sp2x+sqx << "  " << sp2y+sqy << "  " << sp2z+sqz << endl;
//              cout <<sE2*sE2+2*sE2*sq0+sq0*sq0 - pow((sp2x+sqx),2)-pow((sp2y+sqy),2)-pow(sp2z,2)-2*sp2z*(sAA*sq0+sBB)-sAA*sAA*sq0*sq0-2*sAA*sq0*sBB-sBB*sBB <<endl;
              cout << (sAA*sq0+sBB)*(sAA*sq0+sBB) << "  " << sAA*sAA*sq0*sq0+2*sAA*sq0*sBB+sBB*sBB << endl;
              cout << aaa*sq0*sq0+bbb*sq0+ccc << endl;
...
// ..........................................................................................................
    ...
    double calRapidity(double fourP[4]){
        ...
        if(p0<fabs(pz)) {
            cout << "p0=" << p0 << ", px=" << fourP[1] << ", py=" << fourP[2] << ", pz=" << pz << endl;
            cout << "Energy smaller than pz when calculating rapidity!" << endl;
            exit(EXIT_FAILURE);
//            etaflag = 1;
//            return 100;
        }
 
        if(fabs(p0-pz)<epsilon) {
           if(pz>0) return 1000.0;
           else return -1000.0;
        } else {
