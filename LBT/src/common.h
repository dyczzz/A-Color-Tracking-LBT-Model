...
const double cutOut=epsilon;  // neglect light partons in output files for E < cutOut;

//...paramters for the bulk, can be changed from input parameter file
int       vacORmed=1; // 0-vacuum; 1-medium
int       bulkFlag=2; // 0-static medium; 1-OSU hydro; 2-CCNU hydro 
double    temp0=0.25; // medium temperature                 
double    hydro_Tc=0.165;
//...derived quantities
double    temp00; 

int       etaflag=0; //e < pz = 1
...
double    ipTmin=0.0;
double    ipTmax=800.0;
double    eta_cut=0.5;     //cut for sampling initial rapidity
double    cutRapidity=3.0;     //no scattering for particles outside this rapidity range
...
double RHQb[60][20];        //total scattering rate for heavy quark
double RHQ11b[60][20];      //Qq->Qq
double RHQ12b[60][20];      //Qg->Qg
double qhatHQb[60][20];     //qhat of heavy quark
double qhat_over_T3;        //qhat/T^3 for heavy quark as fnc of (T,p)
double D2piT;

// for heavy quark radiation table
///cccconst int HQener_gn=500;
const int HQener_gn=1000;
const int t_gn=75;
const int temp_gn=100;

double dNg_over_dt_c[t_gn+2][temp_gn+1][HQener_gn+1]={0.0};
double dNg_over_dt_b[t_gn+2][temp_gn+1][HQener_gn+1]={0.0};
...
double max_dNgfnc_b[t_gn+2][temp_gn+1][HQener_gn+1]={0.0};
double max_dNgfnc_q[t_gn+2][temp_gn+1][HQener_gn+1]={0.0};
double max_dNgfnc_g[t_gn+2][temp_gn+1][HQener_gn+1]={0.0};

///cccconst double HQener_max=1000.0;
const double HQener_max=2000.0;
...
double p_num[dimParList]={0.0};
double p_num0[dimParList]={0.0};
...
///cccconst int N_p1=500;
const int N_p1=1000;
const int N_T=60;
const int N_e2=75;
double distFncB[N_T][N_p1][N_e2],distFncBb[N_T][N_p1][N_e2],distFncF[N_T][N_p1][N_e2],distFncFb[N_T][N_p1][N_e2],distMaxB[N_T][N_p1][N_e2],distMaxBb[N_T][N_p1][N_e2],distMaxF[N_T][N_p1][N_e2],distMaxFb[N_T][N_p1][N_e2];
double distFncBM[N_T][N_p1],distFncBMb[N_T][N_p1],distFncFM[N_T][N_p1],distFncFMb[N_T][N_p1];
double min_p1=0.0;
///cccdouble max_p1=1000.0;
double max_p1=2000.0;
...
//add two variable for qperp output
int nqperp=0;
double qperp[dimParList]={0.0};
...
void collHQ22(int CT,double temp,double qhat,double v0[4],double p0[4],double p2[4],double p3[4],double p4[4],double &qt,int parID);
