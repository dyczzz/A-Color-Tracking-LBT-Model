...
//    ipput_filename2 = "/data/ycdang/particle/LBT/light/new/ptmin=2/pt_hat_500-700/lp-posi."+str+".dat";// output files of final hadrons
...
//    ipput_filename3 = "/data/ycdang/particle/LBT/light/new/ptmin=2/pt_hat_500-700/lp-nega."+str+".dat";
    ...
    std::vector<double> pxpo(100000, 0.0), pypo(100000, 0.0), pzpo(100000, 0.0), epo(100000, 0.0), ptpo(100000, 0.0), xxpo(100000, 0.0), yypo(100000, 0.0), zzpo(100000, 0.0), ttpo(100000, 0.0), phio(100000, 0.0), etao(100000, 0.0), mass(10000, 0.0);
    std::vector<double> pxp(100000, 0.0), pyp(100000, 0.0), pzp(100000, 0.0), ep(100000, 0.0), ptp(100000, 0.0), xxp(100000, 0.0), yyp(100000, 0.0), zzp(100000, 0.0), ttp(100000, 0.0), phi(100000, 0.0), distance(100000, 0.0), Qscale(100000, 0.0);
    std::vector<int> nncol(100000, 0), aacol(100000, 0), index(100000, 0), qindex(100000, 0), aqindex(100000, 0), used(100000, 0), pair(10000, 0), apair(10000, 0), gindex(10000, 0), Snum(10000, 0);
    std::vector<std::vector<int>> strings(10000, std::vector<int>(10000, 0));
    std::vector<std::vector<double>> dsting(10000, std::vector<double>(10000, 0.0));
    // event loop
...
//        cout << eventid << " " << Npart1 <<endl;
        ...
        for (int ll=0;ll<Npart1;ll++) {
                ...
                if (fabs(c_id) > 21 || ((fabs(c_id) < 19)&&(fabs(c_id)>10))) continue;
                idpo[NN]=c_id;
                if(c_id==21){gindex[Ngluon]=NN;Ngluon++;}
                pxpo[NN]=c_px;
                pypo[NN]=c_py;
                pzpo[NN]=c_pz;
                ptpo[NN]=c_px*c_px+c_py*c_py;
...
//                nncol[ll]=c_c;
//                aacol[ll]=c_ac;
                if(ll<partonid1) Qscale[NN]=pT_cut; else Qscale[NN]=0.0;
                double aamid=0.5*log((pmg+c_pz)/(pmg-c_pz));
                etao[NN]=aamid;
                phio[NN]=atan2(c_py,c_px);
                index[NN]=NN;
                used[NN]=0;
                NN++;
        }
...
//                aacol[ll]=c_c;
        ...
        mmaxindex=searchmax(ptpo.data(), index.data(), Npart);// get the leading parton
...
//        cout<<"================================"<<endl;
        ...
        for (int ll=0;ll<Npart;ll++) {
            if ((idpo[ll]<=7)&((idpo[ll])>0)) {
                qindex[Nquark]=ll;Nquark++;
            }
            if (((idpo[ll]>-7)&((idpo[ll])<0))||idpo[ll]>1000) {
       ...
       //Nquark>Naquark>0
        if (Nquark>Naquark) {
            ...
            if(Naquark==1){
                std::vector<int> used_q(Nquark, 0);
                ...
                int minindex=searchmin(distance.data(), qindex.data(), used_q.data(), Nquark);
                ...
                std::vector<int> temindex(Naquark, 0);
        ...
        if(Naquark>Nquark){
            // if the only one (anti-)quark
            if(Nquark==1){
                std::vector<int> used_q(Naquark, 0);
                ...
                int minindex=searchmin(distance.data(), aqindex.data(), used_q.data(), Naquark);
                ...
                std::vector<int> temindex(Nquark, 0);
                for(int yy=0;yy<Nquark;yy++){
        ...
        if(Naquark==Nquark){
            if(Nquark>1){//cout<<"df";
                ...
                for(int www=0;www<Nquark;www++){
                    std::vector<std::vector<double>> disquark(Nquark, std::vector<double>(Naquark, 0.0));
        ...
        if(Ngluon>0){
            for(int pp=0;pp<Ngluon;pp++){
                std::vector<double> disg(Npair, 0.0), disq(Npair, 0.0);
                ...
                int minindex=searchmin2(disg.data(), Npair);//minindex belongs to Npair
        ...
        for(int kkk=0;kkk<Npair;kkk++){
            if(Snum[kkk]>=2){
                std::vector<int> temindex(Snum[kkk], 0);
...
//        pythia.forceHadronLevel();
        ...
        if(simble>0){
            output2 << iEvent+1<<" "<<simble << " " << weight <<" "<< weightSum << endl;
            for(int i=0; i<pythia.event.size();i++)
                {
                if (pythia.event[i].isFinal() ){
                    c_id = pythia.event[i].id();
